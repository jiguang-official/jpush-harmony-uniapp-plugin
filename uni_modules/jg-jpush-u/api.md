...
默认任何时间都允许推送。调用此 API 可设置允许推送的时间段：仅在该时间段内会展示通知，其余时间收到的通知会被丢弃（纯客户端逻辑，**仅对通知有效，自定义消息不受影响**）。

- **Android**：已实现，行为与极光 Android 文档一致。
...
- weekDays
    - 允许推送的星期：0=周日、1=周一…6=周六；**null**=任何时间都可收到；**空数组 []**=任何时间都收不到。
- startHour
